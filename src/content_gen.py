...
이 블로그 운영자는 해당 직업을 직접 경험한 사람이 아닙니다.

후기형·경험담형·인터뷰형 작성 시 반드시 아래 방식 사용:

금지:
- "제가 직접 해보니", "저는 이 일을 하면서" 등 1인칭 직접 경험
- 마치 본인이 그 직업을 가진 것처럼 쓰는 방식

권장 표현:
- "이 일을 하는 분들 얘기를 들어보면"
- "현직자들이 공통적으로 하는 말이"
- "실제로 이 직업에 종사하는 사람들에 따르면"
- "커뮤니티에서 이 직업 종사자들이 자주 하는 말이"
- "이 일을 오래 한 분들이 이렇게 얘기하더라고요"
- "주변에 이 직업을 가진 분들 얘기를 들어보면"

자연스러운 제3자 시점으로 현실감 있게 전달합니다.
...
인센티브·보너스가 있는 직업은 연봉표 바로 아래에 추가 설명:
<div style="background:#f6ffed;border-left:4px solid #52c41a;padding:16px;margin:12px 0;border-radius:4px;">
  <strong>💰 인센티브 · 보너스</strong><br>
  어떤 인센티브가 있는지, 지급 조건/기준, 실제로 얼마나 받을 수 있는지 구체적으로 설명
</div>

인센티브가 있는 직업 예시: 영업직, 금융(펀드매니저·애널리스트), IT(스타트업 스톡옵션), 보험설계사, 부동산중개사, 의사(성과급), 호텔·서비스직(팁) 등
인센티브가 없거나 미미한 직업(공무원, 교사 등)은 이 박스 생략.
...
독자가 스캔할 때 핵심만 바로 보이도록 <strong> 태그로 강조합니다.

반드시 굵게 처리할 정보:
- 연봉 수치 (예: <strong>평균 5,200만원</strong>)
- 자격증·면허명 (예: <strong>정보보안기사</strong>)
- 필수 학력·경력 조건
- 고용 전망 키워드 (예: <strong>증가 추세</strong>)
- 핵심 업무 키워드 1~2개 per 문단
- 취업 준비 단계의 핵심 항목
- 독자가 반드시 알아야 할 현실적 수치

규칙:
- 문단마다 1~2개만 강조 (남발 금지)
- 박스/표 안에서도 핵심 수치·단어는 강조
- 단순 꾸밈 목적 금지 — 실제로 중요한 정보만 강조
...
- 연봉 수치, 자격증명, 핵심 조건 등 중요 정보는 <strong> 태그로 강조
- 문단마다 굵게 처리 1~2개 (남발 금지)"""
